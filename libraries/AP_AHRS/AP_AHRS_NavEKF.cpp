...
void AP_AHRS_NavEKF::update_spfEKF2(void) {
    if (!_spf_ekf2_started) {
        // wait 1 second for DCM to output a valid tilt error estimate
        if (start_time_ms == 0) {
            start_time_ms = AP_HAL::millis();
        }
        if (AP_HAL::millis() - start_time_ms > startup_delay_ms || _force_ekf) {
            _spf_ekf2_started = spfEKF2.InitialiseFilter();
            if (_force_ekf) {
                return;
            }
        }
    ...
    if (_spf_ekf2_started) {
        spfEKF2.UpdateFilter();
        Vector3f eulers;
        spfEKF2.getRotationBodyToNED(_spf_dcm_matrix);
        spfEKF2.getEulerAngles(-1, eulers);
        roll = eulers.x;
        pitch = eulers.y;
        yaw = eulers.z;
        ...
        // Use the primary EKF to select the primary gyro
        const int8_t primary_imu = spfEKF2.getPrimaryCoreIMUIndex();
        ...
        // Note sign convention used by EKF is bias = measurement - truth
        _gyro_drift.zero();
        spfEKF2.getGyroBias(-1, _gyro_drift);
        _gyro_drift = -_gyro_drift;
        ...
        for (uint8_t i = 0; i < _ins.get_accel_count(); i++) {
            ...
            if (_ins.get_accel_health(i)) {
                _accel_ef_spf_ekf[i] = _spf_dcm_matrix * get_rotation_autopilot_body_to_vehicle_body() * accel;
            }
        }
        _accel_ef_spf_ekf_blended = _accel_ef_spf_ekf[primary_imu >= 0 ? primary_imu : _ins.get_primary_accel()];
//        nav_filter_status filt_state;
//        spfEKF2.getFilterStatus(-1, filt_state);
//        AP_Notify::flags.gps_fusion_spf = filt_state.flags.using_gps; // Drives AP_Notify flag for usable GPS.
//        AP_Notify::flags.gps_glitching_spf = filt_state.flags.gps_glitching;
//        AP_Notify::flags.have_pos_abs_spf = filt_state.flags.horiz_pos_abs;
    }
}
...
void AP_AHRS_NavEKF::writeOptFlowMeas(const uint8_t rawFlowQuality, const Vector2f &rawFlowRates,
                                      const Vector2f &rawGyroRates, const uint32_t msecFlowMeas,
...
void AP_AHRS_NavEKF::writeBodyFrameOdom(float quality, const Vector3f &delPos, const Vector3f &delAng, float delTime,
                                        uint32_t timeStamp_ms, const Vector3f &posOffset) {
    EKF3.writeBodyFrameOdom(quality, delPos, delAng, delTime, timeStamp_ms, posOffset);
}

// Write position and quaternion data from an external navigation system
void
AP_AHRS_NavEKF::writeExtNavData(const Vector3f &sensOffset, const Vector3f &pos, const Quaternion &quat, float posErr,
                                float angErr, uint32_t timeStamp_ms, uint32_t resetTime_ms) {
...
bool AP_AHRS_NavEKF::get_innovations(Vector3f &velInnov, Vector3f &posInnov, Vector3f &magInnov, float &tasInnov,
...
bool AP_AHRS_NavEKF::get_variances(float &velVar, float &posVar, float &hgtVar, Vector3f &magVar, float &tasVar,
